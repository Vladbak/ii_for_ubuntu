...
!define interface_exec "InfinityIsland.exe"
;!define stack_manager_exec "stack-manager.exe"
!define interface_icon "UTII.ico"
...
!define install_dir_company "$PROGRAMFILES\${install_directory}"
; use this for 64 bit:
...
;Section "Registry Entries and Procotol Handler" SEC01
...
; create shortcuts
Section "Desktop and Start Menu Shortcuts" SEC02
    ...
    CreateShortCut "${startmenu_company}\Infinity Island.lnk" "$INSTDIR\${interface_exec}" "" "$INSTDIR\${interface_icon}"
    ...
    ; Desktop icon:
    CreateShortCut "$DESKTOP\${company}.lnk" "$INSTDIR\${interface_exec}" "" "$INSTDIR\${interface_icon}"
SectionEnd
